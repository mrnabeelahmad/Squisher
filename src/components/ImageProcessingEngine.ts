...
import { PipelineStep, CropAnchor, AspectRatioType, RemoveBgStepConfig } from '../types';

/**
 * Helper to convert a Hex string to custom RGB details.
 */
...
/**
 * Executes high-performance, smart client-side background keying.
 */
export function executeBackgroundRemoval(
  ctx: CanvasRenderingContext2D,
  width: number,
  height: number,
  config: RemoveBgStepConfig
): HTMLCanvasElement {
  ...
  if (config.method === 'ai-dominant') {
    // Collect sampling from borders to analyze the true dominant background shade
    const sampleSize = 16;
    const samples: { r: number; g: number; b: number }[] = [];
    
    // Top & Bottom edges sampler
    for (let x = 0; x < width; x += Math.max(1, Math.floor(width / sampleSize))) {
      const idxTop = x * 4;
      const idxBottom = (height - 1) * width * 4 + x * 4;
      if (idxTop < data.length) samples.push({ r: data[idxTop], g: data[idxTop + 1], b: data[idxTop + 2] });
      if (idxBottom < data.length) samples.push({ r: data[idxBottom], g: data[idxBottom + 1], b: data[idxBottom + 2] });
    }
    // Left & Right edges sampler
    for (let y = 0; y < height; y += Math.max(1, Math.floor(height / sampleSize))) {
      const idxLeft = y * width * 4;
      const idxRight = y * width * 4 + (width - 1) * 4;
      if (idxLeft < data.length) samples.push({ r: data[idxLeft], g: data[idxLeft + 1], b: data[idxLeft + 2] });
      if (idxRight < data.length) samples.push({ r: data[idxRight], g: data[idxRight + 1], b: data[idxRight + 2] });
    }

    if (samples.length > 0) {
      // Find cluster/average of samples
      ...
      keyR = Math.round(sumR / samples.length);
      keyG = Math.round(sumG / samples.length);
      keyB = Math.round(sumB / samples.length);
    }
  } else if (config.method === 'chroma-key') {
    const rgb = hexToRgb(config.chromaColor);
    keyR = rgb.r;
    keyG = rgb.g;
    keyB = rgb.b;
  } else if (config.method === 'border-bleed') {
    // Focus key targeting the absolute top-left pixel color as original backdrop paint
    keyR = data[0];
    keyG = data[1];
    keyB = data[2];
  } else if (config.method === 'luminance') {
    // Look for high luminance white backdrops
    keyR = 255;
    keyG = 255;
    keyB = 255;
  }

  // 2. Compute tolerances and apply background mask
  const tolerance = config.tolerance; // Scale typical of 0 - 120
  const feather = Math.max(1, config.feather); // 1 - 40
  const decontaminate = config.decontaminate;

  // We loop over each pixel and compute Euclidean color distance to key color
  for (let i = 0; i < data.length; i += 4) {
    ...
    if (config.method === 'luminance') {
      // Calculate standard ITU BT.709 luminance
      const luma = 0.2126 * r + 0.7152 * g + 0.0722 * b;
      // Distance is how close it is to white
      distance = 255 - luma;
    } else {
    ...
    if (distance <= tolerance) {
      // Inside absolute background area
      alphaMultiplier = 0.0;
    } else if (distance < tolerance + feather) {
      // Intermediate feathered zone (linear interpolant mapping details)
      alphaMultiplier = (distance - tolerance) / feather;
    } else {
      // Keep foreground
      alphaMultiplier = 1.0;
    }

    const newAlpha = Math.round(originalAlpha * alphaMultiplier);
    data[i + 3] = newAlpha;

    // Standard high-fidelity decontamination to avoid coloring edge glow / background leak
    if (decontaminate && alphaMultiplier < 1.0 && alphaMultiplier > 0.0) {
      const scale = alphaMultiplier;
      data[i] = Math.max(0, Math.min(255, Math.round(r * scale + (1 - scale) * 128)));
      data[i + 1] = Math.max(0, Math.min(255, Math.round(g * scale + (1 - scale) * 128)));
      data[i + 2] = Math.max(0, Math.min(255, Math.round(b * scale + (1 - scale) * 128)));
    }
  }
  ...
  if (!processedCtx) return processedCanvas;

  processedCtx.putImageData(imgData, 0, 0);

  // 3. Draw Background Replacement if solid or gradient option is selected
  if (config.replacementType !== 'transparent') {
    const finalCanvas = document.createElement('canvas');
    finalCanvas.width = width;
    finalCanvas.height = height;
    const finalCtx = finalCanvas.getContext('2d');
    if (finalCtx) {
      if (config.replacementType === 'solid') {
        finalCtx.fillStyle = config.replaceSolidColor || '#ffffff';
        finalCtx.fillRect(0, 0, width, height);
      } else if (config.replacementType === 'gradient') {
        const grad = finalCtx.createLinearGradient(0, 0, width, height);
        grad.addColorStop(0, config.replaceGradientStart || '#3b82f6');
        grad.addColorStop(1, config.replaceGradientEnd || '#8b5cf6');
...
export async function processPipeline(
  ...
  for (const step of steps) {
    ...
    switch (step.type) {
      ...
      case 'remove-bg': {
        const cfg = step.removeBgConfig;
        if (!cfg) break;

        const bgCanvas = executeBackgroundRemoval(ctx, currentWidth, currentHeight, cfg);
